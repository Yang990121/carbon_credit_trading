...
function ProjectGenerator({ selectedAccount, role }) {
    ...
    var tableHeaders = ["projectId", "companyInfo", "projectTitle", "projectDescription", "emissionsOffset", "creditsIssued", "financialInfo", "projectDocumentation", "purchaseStatus", "verificationStatus"];
    if (role === "company") {
        tableHeaders = ["projectId", "companyInfo", "projectTitle", "projectDescription", "creditsIssued"];
    } else if (role === "certifier") {
        tableHeaders = ["projectId", "companyInfo", "projectTitle", "projectDescription", "emissionsOffset", "creditsIssued", "financialInfo", "projectDocumentation", "verificationStatus"];
    }
    ...
    const getProjectsByStatus = async (_verificationStatus, _purchaseStatus) => {
        if (projectContract) {
            try {
    ...
    const filterProjects = (status) => {
        if (status === '') {
            ...
            console.log(status)
            const filtered = projects.filter((project) => project.verificationStatus === status);
                        ...
                        {filteredProjects ? filteredProjects.map((project, index) => (
                            ...
                            <TableRow key={index}>
                                {tableHeaders.map((header, headerIndex) => (
                                    <TableCell key={headerIndex} style={{ color: getStatusColor(header, project[header]) }}>{project[header]}</TableCell>
                                ))}
                            </TableRow>
                        )) : null}
                    </TableBody>
